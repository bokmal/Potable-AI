...
- [x] **(실제 확인됨, 수정함)** 사용자 프롬프트를 명령줄 인자로 넘겼더니,
      입력 문장에 괄호 같은 cmd.exe 특수문자가 섞이면(대화 모드 지시문에도
      실수로 괄호가 있었음) 명령줄 자체가 깨져서 엉뚱한 텍스트가 전달되는
      문제가 실사용 중 확인됐다(원래 질문 대신 지시문 일부만 전달됨). 사용자
      프롬프트는 stdin으로 전달하도록 바꿔 해결했다 — stdin은 셸 파싱을
      거치지 않으므로 사용자가 뭘 입력하든 안전하다 (`claudeBridge.js`).
