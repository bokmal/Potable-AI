...
│   ├── store.js             작업 기록(SESSION/TASK/LOG) JSON 저장소 (원자적 쓰기, 사용량 집계)
...
- **사용량 위젯** — 오늘/최근 5시간/최근 7일 요청 수와 그 창의 예상 초기화
  시각을 보여준다. **이건 이 USB에 저장된 로컬 기록만으로 만든 추정치다** —
  Claude Code CLI에서 실제 계정 한도/초기화 시각을 조회하는 공식적인 방법이
  확인되지 않아서, 실제 Anthropic 계정 한도와는 다를 수 있다.
...
- 사용량 위젯은 위에 적었듯 로컬 추정치이며 공식 한도 조회가 아니다.
